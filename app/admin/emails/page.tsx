...
import { Mail, Send, Reply, Trash2, Search, RefreshCw, X, Loader2, Settings, Check, AlertCircle } from "lucide-react"
...
export default function EmailsPage() {
  ...
  return (
    <div>
      {/* tab bar */}
      <div className="flex items-center gap-1 mb-4 p-1 rounded-lg" style={{ backgroundColor: "var(--input-bg)", width: "fit-content" }}>
        <button onClick={() => setTab("inbox")} className="flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-all" style={{ backgroundColor: tab === "inbox" ? "var(--accent-bg)" : "transparent", color: tab === "inbox" ? "var(--accent)" : "var(--text-muted)" }}>
          <Mail size={15} /> Inbox
        </button>
        <button onClick={() => setTab("settings")} className="flex items-center gap-1.5 px-4 py-1.5 rounded-md text-sm font-medium transition-all" style={{ backgroundColor: tab === "settings" ? "var(--accent-bg)" : "transparent", color: tab === "settings" ? "var(--accent)" : "var(--text-muted)" }}>
          <Settings size={15} /> Settings
        </button>
      </div>

      {tab === "inbox" ? (
        <div className="flex h-[calc(100vh-12rem)] gap-0 rounded-xl overflow-hidden" style={{ border: "1px solid var(--card-border)", backgroundColor: "var(--card-bg)" }}>
          {/* left panel */}
          <div className="w-96 flex-shrink-0 flex flex-col" style={{ borderRight: "1px solid var(--card-border)" }}>
            <div className="flex items-center gap-2 p-3" style={{ borderBottom: "1px solid var(--card-border)" }}>
              <button onClick={() => setShowCompose(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all" style={{ backgroundColor: "var(--accent)", color: "#fff" }}>
              ...
              <div ref={composeRef} className="w-full max-w-lg rounded-xl shadow-2xl" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--card-border)" }} onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSend} className="p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <button type="submit" disabled={sending} className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-50" style={{ backgroundColor: "var(--accent)", color: "#fff" }}>
        ...
        <div className="max-w-xl mx-auto mt-8">
          <div className="rounded-xl p-6" style={{ backgroundColor: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
            <div className="flex items-center gap-3 mb-6">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg" style={{ backgroundColor: "var(--accent-bg)" }}>
                <Settings size={18} style={{ color: "var(--accent)" }} />
              </div>
              <div>
                <h2 className="text-base font-semibold" style={{ color: "var(--text-primary)" }}>Email Connection</h2>
                <p className="text-xs" style={{ color: "var(--text-muted)" }}>Configure IMAP settings for the inbox to fetch emails</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={imapHost} onChange={e => setImapHost(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={{ backgroundColor: "var(--input-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }} />
              </div>
              <div>
                <label className="block text-xs font-medium uppercase tracking-wider mb-1.5" style={{ color: "var(--text-muted)" }}>IMAP Port</label>
                <input value={imapPort} onChange={e => setImapPort(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={{ backgroundColor: "var(--input-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }} />
              </div>
              <div>
                <label className="block text-xs font-medium uppercase tracking-wider mb-1.5" style={{ color: "var(--text-muted)" }}>IMAP Username</label>
                <input value={imapUser} onChange={e => setImapUser(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={{ backgroundColor: "var(--input-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }} />
              </div>
              <div>
                <label className="block text-xs font-medium uppercase tracking-wider mb-1.5" style={{ color: "var(--text-muted)" }}>IMAP Password</label>
                <input type="password" value={imapPass} onChange={e => setImapPass(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={{ backgroundColor: "var(--input-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)" }} />
              </div>
            </div>

            <div className="flex items-center justify-between mt-6 pt-4" style={{ borderTop: "1px solid var(--card-border)" }}>
              <p className="text-xs" style={{ color: "var(--text-muted)" }}>Saved to database. Falls back to env vars.</p>
              <button onClick={handleSaveSettings} disabled={saving} className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-50" style={{ backgroundColor: "var(--accent)", color: "#fff" }}>
                {saving ? <><Loader2 size={14} className="animate-spin" /> Saving...</> : saved ? <><Check size={14} /> Saved</> : <><RefreshCw size={14} /> Sync</>}
              </button>
            </div>
          </div>
        </div>
