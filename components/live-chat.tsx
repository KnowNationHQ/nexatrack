...
export default function LiveChat({ role }: { role: "admin" | "merchant" | "driver" }) {
  ...
  useEffect(() => {
    loadMessages()
    const channel = supabase.channel(`${role}-chat`)
  ...
  async function sendMessage() {
    ...
    await db("livechat_messages", "insert", { data: { sender_role: role, sender_id: user?.id, message: text } })
    setText("")
  }
      ...
      <Card style={{borderColor:'var(--card-border)',backgroundColor:'var(--card-bg)'}} className="flex flex-1 flex-col">
        ...
        <CardContent className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto space-y-2 mb-4">
            {messages.map((m, i) => (
              <div key={m.id || i} className={`flex ${m.sender_role === role ? "justify-end" : "justify-start"}`}>
                <div className="max-w-[70%] rounded-lg px-3 py-2 text-sm"
                  style={m.sender_role === role ? {backgroundColor:'#FF3E41',color:'var(--text-primary)'} : {backgroundColor:'var(--input-bg)',color:'var(--text-muted)'}}>
                  <p>{m.message}</p>
