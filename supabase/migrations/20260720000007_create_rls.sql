-- Helper: get role from profiles table (enables RLS without JWT hook)
...
CREATE POLICY merchant_own_profile ON profiles FOR ALL TO authenticated
  USING (id = auth.uid() AND public.get_user_role() = 'merchant')
  WITH CHECK (id = auth.uid() AND public.get_user_role() = 'merchant');

CREATE POLICY driver_own_profile ON profiles FOR SELECT TO authenticated
  USING (id = auth.uid() AND public.get_user_role() = 'driver');
...
CREATE POLICY merchant_own_parcels ON parcels FOR ALL TO authenticated
  USING (merchant_id = auth.uid() AND public.get_user_role() = 'merchant')
  WITH CHECK (merchant_id = auth.uid() AND public.get_user_role() = 'merchant');
...
CREATE POLICY merchant_own_transactions ON transactions FOR SELECT TO authenticated
  USING (wallet_id IN (SELECT id FROM wallets WHERE merchant_id = auth.uid()) AND public.get_user_role() = 'merchant');
...
CREATE POLICY merchant_own_replies ON ticket_replies FOR ALL TO authenticated
  USING (sender_id = auth.uid() AND public.get_user_role() = 'merchant')
  WITH CHECK (sender_id = auth.uid() AND public.get_user_role() = 'merchant');
